# Save Cache to S3: Inputs & AWS Connector Mapping

This document traces the CI Manager implementation of the **Save Cache to S3** step, its user-facing inputs, AWS connector auth flows, connector → plugin env mapping, and infra-type applicability.
...
- `key` and `sourcePaths` are `@NotNull` in the spec, but become optional when the step identifier is the implicit Harness cache step (`save-cache-harness`).
  - Spec fields: `879-pipeline-ci-commons/src/main/java/io/harness/beans/steps/stepinfo/SaveCacheS3StepInfo.java:82`
  - Optional handling for `save-cache-harness`: `877-pipeline-ci-cd-commons/src/java/io/harness/iacm/execution/PluginSettingUtils.java:1938`
...
- `region` is optional in the spec, but the env builder resolves it with `required=true` before setting it as optional.
  - Spec field: `879-pipeline-ci-commons/src/main/java/io/harness/beans/steps/stepinfo/SaveCacheS3StepInfo.java:88`
  - Builder resolution: `877-pipeline-ci-cd-commons/src/java/io/harness/iacm/execution/PluginSettingUtils.java:1970`
- AWS manual credentials include `sessionTokenRef`, but Save Cache to S3 does not map it into plugin envs.
...
- Cross-account `externalId` is supported by the connector, but Save Cache to S3 does not map `AWS_CROSS_ACCOUNT_EXTERNAL_ID`.
  ...
  - Env name mapping for Save Cache S3 lacks external ID: `877-pipeline-ci-cd-commons/src/java/io/harness/iacm/execution/PluginSettingUtils.java:698`
...
- `332-ci-manager/service/src/main/java/io/harness/ci/execution/plan/creator/CIPipelineServiceInfoProvider.java:300`

Step node wiring to the spec:
- `877-pipeline-ci-cd-commons/src/java/io/harness/beans/steps/nodes/SaveCacheS3Node.java:41`
- `877-pipeline-ci-cd-commons/src/java/io/harness/beans/steps/nodes/SaveCacheS3Node.java:51`

Plan creator:
- `332-ci-manager/service/src/main/java/io/harness/ci/execution/plancreator/SaveCacheS3StepPlanCreator.java:19`

Executor (engine step):
- `332-ci-manager/service/src/main/java/io/harness/ci/execution/registrars/ExecutionRegistrar.java:97`
- `332-ci-manager/service/src/main/java/io/harness/ci/execution/states/SaveCacheS3Step.java:16`

User-facing spec model (inputs live here):
- `879-pipeline-ci-commons/src/main/java/io/harness/beans/steps/stepinfo/SaveCacheS3StepInfo.java:52`

Default retry value:
- `879-pipeline-ci-commons/src/main/java/io/harness/beans/steps/stepinfo/SaveCacheS3StepInfo.java:64`

Step env assembly entry point for Save Cache to S3:
- `877-pipeline-ci-cd-commons/src/java/io/harness/iacm/execution/PluginSettingUtils.java:376`
- `877-pipeline-ci-cd-commons/src/java/io/harness/iacm/execution/PluginSettingUtils.java:1934`

Plugin images configured for Save Cache to S3:
...
- Save Cache S3 secret env name mapping: `877-pipeline-ci-cd-commons/src/java/io/harness/iacm/execution/PluginSettingUtils.java:698`
...
- OIDC envs are merged into Save Cache S3 step envs early: `877-pipeline-ci-cd-commons/src/java/io/harness/iacm/execution/PluginSettingUtils.java:1957`
...
├── Mapped to Plugin Env/Args: PLUGIN_ASSUME_ROLE (externalId not mapped for Save Cache S3)
...
Primary Save Cache S3 env assembly:
- `877-pipeline-ci-cd-commons/src/java/io/harness/iacm/execution/PluginSettingUtils.java:1934`
...
| key | string | Yes | Cache key | `PLUGIN_CACHE_KEY` (`PluginSettingUtils.java:1950`) | 
| bucket | string | Yes | S3 bucket | `PLUGIN_BUCKET` (`PluginSettingUtils.java:1961`) |
| sourcePaths | list<string> | Yes | Paths to archive | `PLUGIN_MOUNT` (`PluginSettingUtils.java:1952`) |
| region | string | No | AWS region | `PLUGIN_REGION` (`PluginSettingUtils.java:1970`) |
| endpoint | string | No | S3 endpoint override | `PLUGIN_ENDPOINT` (`PluginSettingUtils.java:1965`) |
| pathStyle | boolean | No | Force path-style addressing | `PLUGIN_PATH_STYLE` (`PluginSettingUtils.java:1979`) |
| override | boolean | No | Overwrite existing cache | `PLUGIN_OVERRIDE` (`PluginSettingUtils.java:1982`) |
| archiveFormat | enum | No | Cache archive format | `PLUGIN_ARCHIVE_FORMAT` (`PluginSettingUtils.java:1975`) |
| runAsUser | integer | No | Container user override | VM runtime setting, not plugin env (`VmPluginCompatibleStepSerializer.java:237`) |
| resources | object | No | Container resources | Container runtime setting (`SaveCacheS3StepInfo.java:79`) |
| retry | integer | No | Step retry count | Defaults to 1 (`SaveCacheS3StepInfo.java:64`) |

Additional plugin envs set by the builder:
- `PLUGIN_REBUILD=true`: `PluginSettingUtils.java:1963`
- `PLUGIN_EXIT_CODE=true`: `PluginSettingUtils.java:1985`
- `PLUGIN_BACKEND=s3`: `PluginSettingUtils.java:1986`
- `PLUGIN_BACKEND_OPERATION_TIMEOUT`: `PluginSettingUtils.java:1987`
...
| Manual + Cross-Account | `crossAccountRoleArn`, optional `externalId` | `PLUGIN_ASSUME_ROLE` | External ID not mapped for Save Cache S3: `PluginSettingUtils.java:698` |
...
The Save Cache S3 env builder does not branch on infra type:
- `877-pipeline-ci-cd-commons/src/java/io/harness/iacm/execution/PluginSettingUtils.java:1934`
...
| OIDC env injection (`PLUGIN_ASSUME_ROLE`, `PLUGIN_OIDC_TOKEN_ID`) | Yes | Yes | Yes (via VM path) | OIDC injected before step envs: `PluginSettingUtils.java:1957`, `PluginSettingUtils.java:2458` |
| Core cache plugin envs (`PLUGIN_BUCKET`, `PLUGIN_CACHE_KEY`, `PLUGIN_MOUNT`, etc.) | Yes | Yes | Yes (via VM path) | Assembly in `getSaveCacheS3StepInfoEnvVariables`: `PluginSettingUtils.java:1934` |

## Key Takeaways

- Save Cache to S3 uses the AWS connector and supports manual, OIDC, inherit-from-delegate, and IRSA auth flows.
